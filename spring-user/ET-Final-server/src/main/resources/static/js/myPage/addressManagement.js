...
document.addEventListener("DOMContentLoaded", function () {
  ...
  addButton.addEventListener("click", function () {
    ...
    document
      .getElementById("adsPhoneMessage")
      .classList.remove("error", "confirm");
    document
      .getElementById("adsExtraPhoneMessage")
      .classList.remove("error", "confirm");
  ...
  addressForm.addEventListener("submit", async function (e) {
    ...
    // 추가 연락처
    const extraPhoneValue = document
      .getElementById("adsExtraPhone")
      .value.trim();
    ...
    try {
      ...
      if (response.ok) {
        ...
        addressItem.innerHTML = `
								...
								<p>${formData.phone}${
          formData.extraPhone ? ` / ${formData.extraPhone}` : ""
        }</p>
						`;
  ...
  addresses.forEach((address) => {
    ...
    addressItem.innerHTML = `
            ...
            <div class="address-actions">
                <button class="edit-btn" data-address-no="${
                  address.addressNo
                }">수정</button>
                <button class="delete-btn" data-address-no="${
                  address.addressNo
                }">삭제</button>
            </div>
        `;
          ...
          if (response.ok) {
            ...
            try {
              localStorage.setItem(
                "defaultAddress",
                JSON.stringify(defaultAddressInfo)
              );
              console.log(
                "Default address saved to localStorage:",
                defaultAddressInfo
              );
            } catch (storageError) {
    ...
    checkbox.addEventListener("change", (e) => {
      ...
      allCheckboxes.forEach((cb) => {
        if (cb !== e.target) {
          cb.checked = false;
          cb.closest(".address-item")
            .querySelector(".address-actions")
...
function openEditModal(address) {
  ...
  document
    .getElementById("editPhoneMessage")
    .classList.remove("error", "confirm");
  document
    .getElementById("editExtraPhoneMessage")
    .classList.remove("error", "confirm");
