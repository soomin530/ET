document.addEventListener("DOMContentLoaded", async () => {
  ...
  function generateSeatsFromData(seatData) {
    ...
    seatData.forEach((data) => {
      ...
      // 조건에 따라 석을 붙임
      const gradeName =
        data.GRADENAME === "전석" ? "전석" : `${data.GRADENAME}석`;
      ...
      const maxSeatsPerRow =
        data.TOTALSEATCOUNT <= 50 ? 10 : data.TOTALSEATCOUNT > 200 ? 25 : 20;
      ...
      for (let row = 1; row <= rows; row++) {
        ...
        for (
          let col = 1;
          col <= maxSeatsPerRow && seatNumber <= data.TOTALSEATCOUNT;
          col++
        ) {
  ...
  function updateSelectedSeatsInfo() {
    ...
    selectedSeatsElement.textContent =
      displaySeatNumbers.length > 0 ? displaySeatNumbers.join(", ") : "없음";
  ...
  async function checkAndSetDefaultAddress() {
    ...
    if (!defaultAddress) {
      console.log(
        "로컬 스토리지에 기본 배송지가 없습니다. 서버에서 가져옵니다..."
      );
      try {
  ...
  document.getElementById("confirm-btn").addEventListener("click", () => {
    ...
    if (!accessToken) {
      alert("주문자 정보를 확인 하려면 로그인이 필요합니다.");
      window.close(); // 창 닫기
